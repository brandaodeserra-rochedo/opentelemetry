...
processor = BatchLogRecordProcessor(OTLPLogExporter(endpoint='http://localhost:4317', insecure=True))
...
def sum():
    with tracer.start_as_current_span("calc-tarifacao"):
        logger.info("Iniciando a operação de tarifação")
        result_sum = 1 + 1
        logger.info(f"Sum ok with result = {result_sum}")

def multiply():
    with tracer.start_as_current_span("star-sap-integration"):
        logger.info("Iniciando a operação de integração com SAP")
        result_multiply = 2 * 2
        logger.info(f"ok with result = {result_multiply}")

def substract():
    with tracer.start_as_current_span("publish-kafka-saut"):
        logger.info("Iniciando publicação no kafka SUAT")
        result_substract = 10-5
        logger.info(f"ok with result = {result_substract}")

def divide():
    with tracer.start_as_current_span("saut-integration"):
        logger.info("Error de integração com SAUT")
        print(1 / 0)
